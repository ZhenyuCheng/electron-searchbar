<template>
    <div v-if="iframeUrl"
        :style="{width: `${iframe.width }px`, height: `${iframe.height}px`}">
        ...
        <collectModal :visible.sync="showCollectModal"
            :iframe="iframe"></collectModal>
        <a class="btn-floating waves-effect waves-light blue back"
            @click="backToList">
            <i class="iconfont iconfanhui" />
        </a>
        <a class="btn-floating waves-effect waves-light gray switch"
            @click="changeScale">
            <i class="iconfont iconbanbenqiehuan" />
        </a>
        <a class="btn-floating waves-effect waves-light red collect"
            @click.stop="collectSite">
            <i class="iconfont iconshoucang" />
        </a>
        <div v-if="loading"
            class="loading">
            <img src="../../../static/assets/loading.svg"
                alt="loading" />
        </div>
    </div>
    <div v-else>
        <div v-for="(item, index) in list"
            ...
            <div class="icon-wrap">
                <span v-if="item.iconloaded !== true"
                    :style="{background: color[index % color.length]}">{{ item.name[0].toUpperCase() }}</span>

                    <img v-if="item.iconloaded !== false"
            ...
            <div class="info-wrap">
                <div class="name">{{ item.name }}</div>
                    <div class="desc f-toe"
                        :title="item.desc || item.url">
                        {{ item.desc || item.url }}</div>
                    </div>
 ...
 * @Last Modified time: 2019-11-07 23:28:56
 */
...
export default {
    ...
    computed: {
        top() {
            return this.iframe.height * (this.scale - 1) / 2 - this.iframe
                .minHeight
        },
        left() {
